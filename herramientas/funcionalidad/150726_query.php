...
function quinielasUsu($jornada){
	$sql    = 'SELECT p.cve_partido part, case when comodin is null then resultado_quiniela else concat(resultado_quiniela," - ",comodin) end resul,u.idusu usu,u.nom_user,q.resultado_real FROM quiniela q INNER JOIN partido p ON q.Partido_cve_partido = p.cve_partido INNER JOIN cuentausuario u ON q.Usuario_idusu = u.idusu WHERE q.Jornada_cve_jornada = %d ORDER BY  q.Usuario_idusu,p.fecha,p.hora,p.cve_partido;';
	$con    = new Conexion($GLOBALS['Host'],$GLOBALS['User'],$GLOBALS['Pass'],$GLOBALS['dbName']);
